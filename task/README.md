...
## Builder패턴
### 이펙티즈자바 참조
  - 생성자에 전달되는 인자 수가 많을때 적용 가능한 대안
  - 점층적 생성자 패턴이란 것이 있다. 
    - 인자 수가 늘어나면 클라이언트 코드를 작성하기 어려워지고, 읽기 어려운 코드가 된다. 
  - 자바빈 패턴이 있다. 
    - 인자가 없는 생성자를 호출해 객체부터 만들고, 설정 메서드(setter)들을 호출해 값을 채운다. 
    - 1회의 함수 호출로 객체 생성을 끝낼 수 없는 객체 일관성이 일시적으로 깨지는 문제가 있다. 
  - 자바 빌더 패턴이 있다. 
    - 필요한 객체를 직접 생성하는 대신, 클라이언트는 먼저 필수 인자들을 생성자에 전부 절달해 빌더객체를 만든다. 
    - 그런다음 빌더 객체에 정의된 설정 메서드들을 호출해 선택적 인자들을 추가해 나간다. 
    - 빌더 클래스는 빌더가 만드는 객체 클래스의 정적 멤버 클래스로 정의한다. 
    - 빌더에 정의된 설정 메서드는 빌더 객체 자신을 반환하므로, 설정 메서드를 호출하는 코드는 이어서 쓸 수 있다.
    - 빌더 패턴은 인자가 많은 생성자나 정적 팩터리가 필요한 클래스를 설게할 때, 특히 대부분의 인자가 선택적 인자인 상황에 유용하다.  